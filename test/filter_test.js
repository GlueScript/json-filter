...
describe('Filter', function() {
    ...
    describe('remove', function() {
        ...
        it('should remove exactly matching items', function() {
            var value = [
                {status: 'error'},
                {status: 'success'},
                {status: 'weird'},
            ...
            filter.remove(value, pattern, function(err, result) {
                ...
                assert(result[1]['status'] == 'weird');
            });
        });
        it('should respond with all non-matching items', function() {
            var value = [
                {status: 'error', code: 500},
                {status: 'success', other: 'throng'},
                {status: 'error', code: 404},
        ...
        it('should respond with items that do not match any value in pattern', function() {
            var value = [
                {status: 'error', code: '500'},
                {status: 'success', code: '200'},
                {status: 'error', code: '404'},
